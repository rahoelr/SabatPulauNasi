                            ...
                            <a href="/produk" class="component-categories d-block">
                                <div class="categories-image" style="border-radius: 10px;">
                                    <img src="{{ asset('storage/category_images/' . $category->images) }}" alt=""
                                        class="w-100" style="object-fit: cover; height: 100%; width: 100%;" />
                                </div>
                    ...
                    @if (count($products) > 7)
                        @for ($k = 0; $k < 8; $k++)
                            <div class="col-6 col-md-4 col-lg-3" data-aos="fade-up" data-aos-delay="{{ $i += 100 }}">
                                @php
                                    $images = [$products[$k]->image1, $products[$k]->image2, $products[$k]->image3, $products[$k]->image4];
                                @endphp
                                <a href="/detail_produk/{{ $products[$k]->id }}" class="component-products d-block">
                                    <div class="products-thumbnail">
                                        <div class="products-image" style="background-image: url('{{ asset('storage/product_images/' . $images[0]) }}');">
                                        </div>
                                    </div>
        ...
        <section class="store-new-products">
            <div class="container">
                ...
                <div class="row">
                    ...
                    @if (count($mitras) > 3)
                        @for ($k = 0; $k < 4; $k++)
                            <div class="col-6 col-md-4 col-lg-3" data-aos="fade-up" data-aos-delay="{{ $i += 200 }}">
                                <a href="/mitra" class="component-products d-block">
                                    <div class="products-thumbnail">
                                        <div class="products-image" style="background-image: url('{{ asset('storage/mitra_images/' . $mitras[$k]->images) }}');">
                                        </div>
                                    </div>
                        ...
                        @foreach ($mitras as $mitra)
                            <div class="col-6 col-md-4 col-lg-3" data-aos="fade-up" data-aos-delay="{{ $j += 200 }}">
                                <a href="#" class="component-products d-block">
                                    <div class="products-thumbnail">
                                        <div class="products-image" style="background-image: url('{{ asset('storage/mitra_images/' . $mitra->images) }}');">
                                        </div>
                                    </div>
                        ...
                        @for ($k = 0; $k < 4; $k++)
                            <div class="col-6 col-md-4 col-lg-3" data-aos="fade-up" data-aos-delay="{{ $i += 200 }}">
                                <a href="/detail-artikel/{{ $articles[$k]->id }}" class="component-products d-block">
                                    <div class="products-thumbnail">
                                        <div class="products-image" style="background-image: url('{{ asset('storage/article_images/' . $articles[$k]->images) }}')">
                                        </div>
                                    </div>
                                    <div class="products-text">{{ substr($articles[$k]->title, 0, 30) }}...</div>
                                </a>
                            </div>
                        @endfor
                    @else
                        @foreach ($articles as $article)
                            <div class="col-6 col-md-4 col-lg-3" data-aos="fade-up" data-aos-delay="{{ $j += 200 }}">
                                <a href="/detail-artikel/{{ $article->id }}" class="component-products d-block">
                                    <div class="products-thumbnail">
                                        <div class="products-image" style="background-image: url('{{ asset('storage/article_images/' . $article->images) }}')">
                                        </div>
                                    </div>
                                    <div class="products-text">{{ substr($article->title, 0, 30) }}...</div>
                                </a>
                            </div>
                        @endforeach
                    @endif
            ...
            <div class="container">
                ...
                <form action="{{ route('admin-message.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    @enderror"
                        ...
                        is-invalid
                    @enderror"
                                    id="subject" aria-describedby="emailHelp" name="subject"
                                    value="{{ old('subject') }}" />
                                @error('subject')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                        ...
                        is-invalid
                    @enderror"
                                    id="content" rows="5" aria-describedby="emailHelp" name="content">{{ old('content') }}</textarea>
                                @error('content')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
